...
최적해 집합은
\[
W^*(c)=\arg\min_{w\in S}\{c^\top w\}, \quad w^*(c)\in W^*(c).
\]

Predict-then-Optimize framework에서는 optimal decision \(w^*(\cdot)\)을 구할 때  
진짜 목적함수 계수 \(c\)를 알 수 없기 때문에 ML model이 예측한 \(\hat c\)를 사용함.  
데이터셋 \(\{(x_1,c_1),(x_2,c_2),\dots,(x_n,c_n)\}\)을 기반으로 예측 수행.  
입력 특징 \(x\in\mathbb{R}^{p}\)는 비용 벡터 \(c\)를 예측하기 위한 feature들.  
\(\mathcal{H}\): \(x\)로부터 \(c\)를 예측하는 ML hypothesis class.  
손실 함수 \(\ell:\mathbb{R}^{d}\times\mathbb{R}^{d}\to\mathbb{R}_{+}\) (본 논문에서는 SPO).  
ERM은
\[
...
\]
즉 예측 \(\hat c\)로 최적화를 했을 때 나온 결정이 실제 비용에서 갖는 비용과  
진짜 비용으로 얻을 수 있는 최적 비용의 차이.  
...
## 8) Decision Trees for Decision-Making
...
모든 DT의 목표는 train data를 \(L\)개의 leaf인 \(R_1,\dots,R_L:=R_{1:L}\)로 분할하는 것,  
...
**Theorem 1.**  
리프 \(\ell\) 내 모든 관측값들의 평균 cost vector를
\[
c_\ell \;:=\; \frac{1}{|R_\ell|}\sum_{i\in R_\ell} c_i
\]
라고 할 때, 이에 대응되는 decision problem이 **고유한 최소해**를 가지면  
(\(|W^*(c_\ell)|=1\)) 이는 리프 안에서의 SPO loss를 minimize함, 즉
\[
...
이제 이 식을 어떻게 풀 것인가??  
위의 목적 함수에 대해 SPO 트리를 학습시키는 방법으로 **recursive partitioning** 기법을 제안.  
\(x_{i,j}\): \(i\)번째 학습 sample의 \(j\)번째 feature 값. 트리 분할은 feature \(j\)를 기준으로  
임계값 \(s\)보다 작거나 같은지 여부에 따라 데이터를 두 group으로 나눔:
\[
R_1(j,s)=\{i:x_{i,j}\le s\},\qquad R_2(j,s)=\{i:x_{i,j}> s\}.
\]
...
Theorem 1을 활용하면 목적함수 값은  
1) 분할기준에 따라 학습 데이터를 나누고,  
2) 각 leaf에서 평균 cost 벡터와 이에 대한 최적 결정 벡터를 구하고,  
3) 해당 결정으로 인해 생기는 SPO loss를 계산하고 SPO loss를 모두 합산한 뒤 \(n\)으로 나누어 평균 손실을 구함.  
이는 빠르게 수렴하지만 최적은 보장하지 않으므로 **MILP**를 활용.  
**SPO forests** — SPO tree를 여러 개 앙상블하여 만듦.
